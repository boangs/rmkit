...
static void doInject(QQmlEngine *engine) {
    for (QWindow *w : QGuiApplication::topLevelWindows()) {
        ...
        comp.setData(
            ...
            "    onClicked: {\n"
            // 挂到 MainView (跟随设备方向旋转的容器) 而非物理窗口根:\n"
            // 一路走到 while(r.parent) 顶会越过旋转边界, 落在不旋转的竖屏物理根 ->\n"
            // 横屏时面板仍竖屏 (实测 _rmhAdvancedPanel 1404x1872 在 MainView 之外)。\n"
            "      var r = advItem, host = null;\n"
            "      for (var p = advItem.parent; p; p = p.parent) {\n"
            "        r = p;\n"
