...
工具是 `tools/qmd-tool/` (Go), 一次 `make build` 后产物落 `dist/qmd-tool*`.

1. 改完跑 hash 命中校验:
   ```bash
   cd tools/qmd-tool && make build && cd ../..
   dist/qmd-tool check
   ```
2. 如新引用了 identifier, 跑一遍 `dist/qmd-tool hash -hashtab tools/hashtab qmd-src/foo.qmd > /tmp/x.qmd`
   看 identifier 名是否被替换成 `[[hash]]` (没替换说明 hashtab 里没有, 部署后会
   silent skip 注入).
...
3. CI 全绿才能 merge:
   - `bash -n` + `shellcheck`
   - `qmd-tool check` 全过 (Go, 取代旧 Python qmd_hash_check)
   - `go vet` + `go test` + cross-compile aarch64/armv7 (3 个 Go 项目: ime-go / upload-server-go / tools/qmd-tool)
   - `py_compile` 开发辅助 Python 脚本 (scripts/merge_ts.py / translate_ts.py)
...
- [ ] CI 全绿 (shell + qmd-tool + go + python)
- [ ] 改 systemd 时跑了 systemd-analyze verify
- [ ] 改 .qmd 时 `dist/qmd-tool check` 通过
```
...
| qmd 注入失败但 xochitl 还正常 | hash 不命中 → qmldiff silent skip; 跑 `dist/qmd-tool check` 验证 |
| qmd 注入失败 + xochitl crash | hash 不命中**且**触发 panic 路径; dist/*.qmd 可能含错误信息而非 .qmd 内容 |
