...
#define ACTIVE_MARKER "/native-touch-active"
#define SCROLL_MARKER "/native-scroll-active"
#define MAX_TOUCH_SLOTS 64
/* [rmkit] 底部上滑手势 → 写导航请求文件, PaperHome 无障碍服务执行回桌面 */
#define NAV_REQUEST_FILE \
    "/android-data/data/com.android.launcher3/files/paper-nav-request"
#define MAX_FRAME_EVENTS 512
static volatile sig_atomic_t stop_requested;
static int log_fd = -1;
static bool touch_marker_active;
...
/*
 * Publish only contact-state transitions.  The display bridge uses this
 * marker to select a quicker monochrome path while a real (non-palm) finger
 * is down.  Avoid rewriting it for every motion report: that would add file
 * system traffic to the hottest input path.
 */
static bool set_touch_active(bool active)
{
    if (touch_marker_active == active)
        return true;
    if (!active) {
        if (unlink(ACTIVE_MARKER) < 0 && errno != ENOENT) {
            log_message("ERROR remove touch activity marker: %s",
                        strerror(errno));
            return false;
        }
        touch_marker_active = false;
        return true;
    }

    int marker = open(ACTIVE_MARKER,
                      O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0644);
    if (marker < 0) {
        log_message("ERROR create touch activity marker: %s",
                    strerror(errno));
        return false;
    }
    const char value[] = "finger-down\n";
    const bool success =
        write(marker, value, sizeof(value) - 1) == sizeof(value) - 1;
    close(marker);
    if (!success) {
        log_message("ERROR write touch activity marker: %s",
                    strerror(errno));
        unlink(ACTIVE_MARKER);
        return false;
    }
    touch_marker_active = true;
    return true;
}

/* [rmkit] 底部上滑回桌面: 状态与识别。fingers=本帧转发手指数, y=最近原始坐标。
 * 条件: 单指、起点在面板底部 4%、上滑超过高度 12%、800ms 内完成;
 * 多指或起点不符则本次按下不再判定。识别成功写请求文件, 由 PaperHome
 * 无障碍服务 (inotify) 执行 GLOBAL_ACTION_HOME。 */
...
static struct swipe_gesture swipe;
...
static void request_nav_action(const char *action)
{
    int fd = open(NAV_REQUEST_FILE,
                  O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0644);
    if (fd < 0) {
        log_message("ERROR nav request open: %s", strerror(errno));
        return;
    }
    (void)!write(fd, action, strlen(action));
    close(fd);
    log_message("bottom swipe-up recognized; nav request: %s", action);
}

/*
 * 手势判定 (SystemUI 导航栏已整体停用, 返回/回桌面均由此识别):
 *   回桌面: 起点在面板底部 4% 内, 上滑超过高度 12%
 *   返回:   起点在左/右边缘 4% 内, 向内横滑超过宽度 12%
 * 都要求单指且 800ms 内完成; 多指或起点不符则本次按下不再判定。
 */
/* [rmkit] 双指按下 → 写滚动标记, 显示桥切快滑模式; 松开即撤 */
static bool scroll_marker_active;
static void set_scroll_active(int fingers)
{
    const bool active = fingers >= 2;
    if (active == scroll_marker_active)
        return;
    if (active) {
        int fd = open(SCROLL_MARKER,
                      O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0644);
        if (fd >= 0) {
            (void)!write(fd, "scroll\n", 7);
            close(fd);
            scroll_marker_active = true;
        }
    } else {
        unlink(SCROLL_MARKER);
        scroll_marker_active = false;
    }
}

static void track_bottom_swipe(int fingers, int x, int y)
{
    if (swipe.abs_max_y <= 0)
        return;
    const int bottom_edge = swipe.abs_max_y - swipe.abs_max_y / 25;
    const int side_edge = swipe.abs_max_x > 0 ? swipe.abs_max_x / 25 : 0;
    const int travel_y = swipe.abs_max_y * 12 / 100;
    const int travel_x = swipe.abs_max_x * 12 / 100;

    if (fingers == 0) {
        if (swipe.active && !swipe.cancelled &&
            monotonic_ms() - swipe.start_ms <= 800u) {
            if (swipe.start_y >= bottom_edge &&
                swipe.start_y - swipe.last_y >= travel_y) {
                request_nav_action("home\n");
            } else if (swipe.abs_max_x > 0 && travel_x > 0 &&
                       ((swipe.start_x <= side_edge &&
                         swipe.last_x - swipe.start_x >= travel_x) ||
                        (swipe.start_x >= swipe.abs_max_x - side_edge &&
                         swipe.start_x - swipe.last_x >= travel_x))) {
                request_nav_action("back\n");
            }
        }
        swipe.active = false;
    ...
    if (!swipe.active) {
        ...
        swipe.cancelled =
            !(y >= bottom_edge ||
              (swipe.abs_max_x > 0 &&
               (x <= side_edge || x >= swipe.abs_max_x - side_edge)));
        swipe.start_x = x;
        swipe.start_y = y;
        swipe.start_ms = monotonic_ms();
    }
    swipe.last_x = x;
    swipe.last_y = y;
}
                              ...
                              int *last_tracking_id)
{
    ...
    if (forwarded_count == 0)
        *last_tracking_id = -1;
    set_scroll_active(forwarded_count);
    track_bottom_swipe(forwarded_count, *last_x, *last_y);
...
int main(int argc, char **argv)
{
    ...
    if (ioctl(physical_fd, EVIOCGABS(ABS_MT_POSITION_Y), &y_info) == 0 &&
        y_info.maximum > 0) {
        swipe.abs_max_y = y_info.maximum;
        if (ioctl(physical_fd, EVIOCGABS(ABS_MT_POSITION_X), &x_info) == 0 &&
            x_info.maximum > 0)
            swipe.abs_max_x = x_info.maximum;
        log_message("nav gestures armed (max_x=%d max_y=%d)",
                    swipe.abs_max_x, swipe.abs_max_y);
    }
    ...
    while (!stop_requested) {
        ...
        for (size_t index = 0; index < event_count; ++index) {
            ...
            if (event->type == EV_SYN &&
                ...
                if (!set_touch_active(last_tracking_id >= 0)) {
                    stop_requested = 1;
                    break;
                }
                frame_event_count = 0;
