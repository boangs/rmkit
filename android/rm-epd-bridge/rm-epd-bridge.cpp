...
constexpr char ScrollActivePath[] = "/native-scroll-active";  // [rmkit] 双指滚动
...
/* [rmkit] v47 HWC damage 透传: 头部 48/52 字节为 'DMG1' 魔数与矩形数 */
constexpr uint32_t DamageTrailerMagic = 0x31474D44;
constexpr uint32_t MaximumTrailerRects = 16;
...
/*
 * [rmkit] 打字时 Trime 每键产生多帧 (按键高亮/候选/正文分别刷), 双核 CPU
 * 被输入法与全帧 diff 抢饥饿。放宽打字帧间隔让 mailbox 合并中间帧, 桥的
 * 处理次数约减半, CPU 让给输入法; 键与候选的最终状态一帧到位。
 */
/* [rmkit] v49: Overlay 直通 + 15fps vsync 后, 110ms 合并窗成了滚动
 * 追手延迟的大头; 会话帧本身已极便宜 (直通+Animate), 收窄到 70ms。 */
constexpr int TypingFrameIntervalMs = 70;
...
constexpr qint64 GhostBudgetScreenMultiples = 15;  // was 5; Android UI accrues ghost area fast, 5 screens fired every ~30 s
...
struct FrameMailbox {
    ...
    /* [rmkit] v47: 与 latest 同步的 HWC damage 脏区 (空列表=本帧无变化) */
    std::vector<QRect> damageRects;
    bool damageKnown = false;
    uint64_t consumedByDisplay = 0;
    std::deque<PenSample> penSamples;
    uint64_t receivedFrames = 0;
    std::chrono::steady_clock::time_point latestFrameArrivedAt{};
...
void receiveHwcFrames(FrameMailbox *mailbox)
{
    ...
    /* [rmkit] v48: 帧缓冲复用池 — 被邮箱挤掉且无人引用的缓冲直接回收,
     * 避免每帧 6.5MB 的分配/释放 (SF 产帧率高于显示消费率时全是白分配)。 */
    std::shared_ptr<QByteArray> spare;
    ...
    while (!stopRequested) {
        ...
        /*
         * [rmkit] v47 HWC damage 透传: 新 HWC 在头部 48/52 写 DMG1 魔数与
         * 矩形数, fd 消息之后跟随矩形表 (x,y,w,h 各 LE i32)。无魔数或
         * count==0 表示无脏区信息, 主循环回退瓦片 diff。
         */
        const uint32_t damageMagic = readLe32(handle, 48);
        const uint32_t damageCount = readLe32(handle, 52);
        std::vector<QRect> damageRects;
        bool damageKnown = false;
        if (damageMagic == DamageTrailerMagic && damageCount > 0) {
            if (damageCount > MaximumTrailerRects) {
                writeLog(QStringLiteral("ERROR HWC damage count %1")
                             .arg(damageCount));
                ::munmap(mapping, bufferBytes);
                ::close(sharedFd);
                break;
            }
            std::array<unsigned char, MaximumTrailerRects * 16> trailer = {};
            if (!readExact(socketFd, trailer.data(),
                           static_cast<size_t>(damageCount) * 16)) {
                writeLog(QStringLiteral("ERROR HWC damage trailer truncated"));
                ::munmap(mapping, bufferBytes);
                ::close(sharedFd);
                break;
            }
            ...
            for (uint32_t i = 0; i < damageCount; ++i) {
                ...
                const auto field = [&trailer, base](size_t at) {
                    return static_cast<int32_t>(
                        static_cast<uint32_t>(trailer[base + at]) |
                        (static_cast<uint32_t>(trailer[base + at + 1]) << 8) |
                        (static_cast<uint32_t>(trailer[base + at + 2]) << 16) |
                        (static_cast<uint32_t>(trailer[base + at + 3]) << 24));
                };
                const QRect rect =
                    QRect(field(0), field(4), field(8), field(12))
                        .intersected(panel);
                if (!rect.isEmpty())
        ...
        if (spare && spare.use_count() == 1 &&
            spare->size() == static_cast<qsizetype>(VisibleBytes)) {
            frame = std::move(spare);
            std::memcpy(frame->data(), mapping, VisibleBytes);
        } else {
            frame = std::make_shared<QByteArray>(
        ...
        {
            const std::lock_guard<std::mutex> lock(mailbox->mutex);
            if (mailbox->receivedFrames != mailbox->consumedByDisplay) {
                /* [rmkit] 邮箱覆盖了未消费帧: 其脏区必须并入本帧, 任一方
                 * 未知则整体退化为未知 (回退 diff), 超限并成包围盒。 */
                if (mailbox->damageKnown && damageKnown) {
                    for (const QRect &kept : mailbox->damageRects)
                        damageRects.push_back(kept);
                    if (damageRects.size() > MaximumTrailerRects) {
                        QRect bounds;
                        for (const QRect &rect : damageRects)
                            bounds = bounds.isEmpty()
                                ? rect : bounds.united(rect);
                        damageRects.assign(1, bounds);
                    }
                } else {
                    damageKnown = false;
                    damageRects.clear();
            ...
            spare = std::move(mailbox->latest);
            mailbox->latest = std::move(frame);
            mailbox->damageRects = std::move(damageRects);
            mailbox->damageKnown = damageKnown;
            ++mailbox->receivedFrames;
            mailbox->latestFrameArrivedAt = std::chrono::steady_clock::now();
        }
        mailbox->changed.notify_one();
    }
                                     ...
                                     const QRect &limit = QRect())
{
    ...
    /* [rmkit] v47: HWC damage 并集限定扫描范围, 范围外像素由 Android
     * 的 surfaceDamage 语义保证未修改, diff 成本与脏区面积成正比。 */
    int tileYBegin = 0;
    int tileYEnd = rows;
    int tileXBegin = 0;
    int tileXEnd = columns;
    if (!limit.isEmpty()) {
        tileXBegin = std::max(0, limit.left() / DamageTileSize);
        tileYBegin = std::max(0, limit.top() / DamageTileSize);
        tileXEnd = std::min(columns, limit.right() / DamageTileSize + 1);
        tileYEnd = std::min(rows, limit.bottom() / DamageTileSize + 1);
    }
                         ...
                         bool inverted)
{
    ...
    /*
     * [rmkit] 行程记忆优化: 文档类内容 90% 是连续相同像素 (整片纸白/
     * 墨黑), 缓存上一像素的输入→输出, 命中时一次 32 位比较直接复用,
     * 实测把大区域转换从 100-300ms 压到 ~20-40ms。语义与原实现一致:
     * 近白吸附 (防 Gallery 3 彩点矩形)、近黑吸附、可选反色。
     */
    uint32_t cachedInput = 0;
    uint32_t cachedOutput = 0;
    bool cacheValid = false;
    for (int y = rect.top(); y <= rect.bottom(); ++y) {
        const size_t rowOffset =
            static_cast<size_t>(y) * AndroidStrideBytes +
            static_cast<size_t>(rect.left()) * BytesPerPixel;
        const unsigned char *in = source + rowOffset;
        unsigned char *out = destination + rowOffset;
        for (int x = rect.left(); x <= rect.right();
             ++x, in += BytesPerPixel, out += BytesPerPixel) {
            uint32_t pixel;
            std::memcpy(&pixel, in, 4);
            if (cacheValid && pixel == cachedInput) {
                std::memcpy(out, &cachedOutput, 4);
                continue;
            }
            const int red = in[0];
            const int green = in[1];
            const int blue = in[2];
            const int maximum = std::max({red, green, blue});
            const int minimum = std::min({red, green, blue});
            const bool paperWhite = minimum >= 250
                && maximum - minimum <= 5;
            ...
            out[0] = static_cast<unsigned char>(
                inverted ? 255 - normalizedRed : normalizedRed);
            out[1] = static_cast<unsigned char>(
                inverted ? 255 - normalizedGreen : normalizedGreen);
            out[2] = static_cast<unsigned char>(
                inverted ? 255 - normalizedBlue : normalizedBlue);
            out[3] = in[3];
            cachedInput = pixel;
            std::memcpy(&cachedOutput, out, 4);
            cacheValid = true;
        }
    }
}
...
struct NativePenOverlay {
    ...
    QRect inkBounds;  // [rmkit] 本轮叠加层累计墨迹范围, 供区域化重置用
    ...
    const NotePenSettings &settings)
{
    ...
    painter.end();
    const QRect clipped = dirty.intersected(
        QRect(0, 96, AndroidWidth, AndroidHeight - 96));
    if (!clipped.isEmpty())
        overlay.inkBounds = overlay.inkBounds.united(clipped);
    return clipped;
}
...
void drawBootSplash(QImage &destination, int step, int steps)
{
    ...
    const int w = destination.width();
    const int cx = w / 2;
    const int cy = destination.height() / 2 - 80;
    ...
    drawRemarkableWordmark(painter, 56, 48 + (60 - 39) / 2, 300);
    QPen divider(QColor(80, 80, 80));
    divider.setWidth(2);
    painter.setPen(divider);
    painter.drawLine(56, 126, w - 56, 126);

    // Android mark (Phosphor "android-logo", regular weight), rendered from
    // the embedded SVG so the artwork can be swapped without touching code.
    static const char kAndroidSvg[] =
        "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 256 256'>"
        "<path fill='#282828' d='M176,148a12,12,0,1,1-12-12A12,12,0,0,1,176,148ZM92,136a12,12,0,1,0,12,12"
        "A12,12,0,0,0,92,136Zm148,24v24a16,16,0,0,1-16,16H32a16,16,0,0,1-16-16V161.13A113.38,113.38,0,0,1,"
        "51.4,78.72L26.34,53.66A8,8,0,0,1,37.66,42.34L63.82,68.5a111.43,111.43,0,0,1,128.55-.19l26-26a8,8,0,"
        "0,1,11.32,11.32L204.82,78.5c.75.71,1.5,1.43,2.24,2.17A111.25,111.25,0,0,1,240,160Zm-16,0a96,96,0,"
        "0,0-96-96h-.34C74.91,64.18,32,107.75,32,161.13V184H224Z'/></svg>";
    QSvgRenderer icon(QByteArray::fromRawData(kAndroidSvg, sizeof(kAndroidSvg) - 1));
    const int iconSize = 220;
    icon.render(&painter, QRectF(cx - iconSize / 2, cy - iconSize / 2 - 20, iconSize, iconSize));

    QFont serif(QStringLiteral("EB Garamond"));
    serif.setPixelSize(52);
    serif.setWeight(QFont::Normal);
    painter.setFont(serif);
    painter.setPen(Qt::black);
    painter.drawText(QRect(0, cy + 120, w, 70), Qt::AlignCenter, QStringLiteral("Android"));

    font.setPixelSize(22);
    font.setWeight(QFont::Normal);
    painter.setFont(font);
    painter.setPen(QColor(110, 110, 110));
    painter.drawText(QRect(0, cy + 200, w, 40), Qt::AlignCenter,
                     uiText("Starting…", "시작 중…", "正在启动…"));

    // Thin progress line, stepped.
    const int barW = 360, barY = cy + 262, barX = cx - barW / 2;
    painter.setPen(QPen(QColor(200, 200, 200), 2));
    painter.drawLine(barX, barY, barX + barW, barY);
    if (step > 0) {
        painter.setPen(QPen(QColor(40, 40, 40), 3));
        painter.drawLine(barX, barY, barX + barW * std::min(step, steps) / std::max(steps, 1), barY);
    }
}
...
/*
 * [rmkit] 息屏壁纸: 复用 stock 的 SleepScreenPath (rmkit 高级面板上传的壁纸,
 * 存在 /home/root/.local/share/rmkit-cn/screens/)。读 xochitl.conf 拿路径,
 * 加载成功则整屏绘制; 失败回落到原有样式。图片按比例缩放居中, 白底。
 */
QString readSleepScreenPath()
{
    QFile conf(QStringLiteral("/home/root/.config/remarkable/xochitl.conf"));
    if (!conf.open(QIODevice::ReadOnly | QIODevice::Text))
        return QString();
    while (!conf.atEnd()) {
        const QByteArray line = conf.readLine().trimmed();
        if (line.startsWith("SleepScreenPath="))
            return QString::fromUtf8(line.mid(16));
    }
    return QString();
}

bool drawSleepWallpaper(QImage &destination)
{
    static QString cachedPath;
    static QImage cachedImage;
    const QString path = readSleepScreenPath();
    if (path.isEmpty())
        return false;
    if (path != cachedPath || cachedImage.isNull()) {
        QImage loaded(path);
        if (loaded.isNull()) {
            static bool warned = false;
            if (!warned) {
                writeLog(QStringLiteral("lock wallpaper load failed: %1")
                            .arg(path));
                warned = true;
            }
            return false;
        ...
        cachedPath = path;
        cachedImage = loaded.convertToFormat(QImage::Format_RGBX8888)
                          .scaled(destination.size(), Qt::KeepAspectRatio,
                                  Qt::SmoothTransformation);
        writeLog(QStringLiteral("lock wallpaper loaded: %1 (%2x%3)")
                    .arg(path)
                    .arg(cachedImage.width())
                    .arg(cachedImage.height()));
    }
    QPainter painter(&destination);
    painter.setCompositionMode(QPainter::CompositionMode_Source);
    painter.fillRect(destination.rect(), Qt::white);
    painter.drawImage(
        QPoint((destination.width() - cachedImage.width()) / 2,
               (destination.height() - cachedImage.height()) / 2),
        cachedImage);
    painter.end();
    return true;
}
...
{
    loadUiLocale();
    if ((style == LockStyle::Classic || style == LockStyle::Fade) &&
        drawSleepWallpaper(destination)) {
        return;
    }
                                ...
                                DisplayProfile profile)
{
    ...
    // Waveform selection now lives in libqsgepaper (ModeMap submit), so the
    // 1-bit Bayer path that used to pair with the fast waveform only produces
    // checkerboard streaks on light-grey UI.  All profiles quantise smoothly.
    if (false && profile == DisplayProfile::Fast) {
...
/*
 * Convert only the 32x32 tiles whose source RGB changed since the previous
 * frame; copy the rest from the previous conversion.  Bayer dithering is
 * per-pixel so tiles convert independently.  A single keyboard key touches
 * ~4 tiles instead of the full 1.6 M-pixel panel, which previously cost
 * 70-100 ms per keystroke on the CPU.
 */
void convertDirtyTilesForEink(const unsigned char *source,
                              const QByteArray &previousSource,
                              const QByteArray &previousOutput,
                              QByteArray &output,
                              bool inverted,
                              DisplayProfile profile,
                              int &convertedTiles,
                              int &totalTiles)
{
    const bool haveBaseline =
        previousSource.size() == static_cast<qsizetype>(VisibleBytes) &&
        previousOutput.size() == static_cast<qsizetype>(VisibleBytes);
    if (!haveBaseline) {
        convertForEink(source, output, inverted, profile);
        const int columns = (AndroidWidth + DamageTileSize - 1) / DamageTileSize;
        const int rows = (AndroidHeight + DamageTileSize - 1) / DamageTileSize;
        convertedTiles = totalTiles = columns * rows;
        return;
    }
    // `output` already holds the previous conversion (it is reused across
    // frames); only ensure it is sized and uniquely owned.  Assigning
    // previousOutput here copied 6.5 MB per frame and cost ~400 ms.
    if (output.size() != static_cast<qsizetype>(VisibleBytes))
        output = previousOutput;
    auto *destination = reinterpret_cast<unsigned char *>(output.data());
    const auto *prev = reinterpret_cast<const unsigned char *>(previousSource.constData());
    const int columns = (AndroidWidth + DamageTileSize - 1) / DamageTileSize;
    const int rows = (AndroidHeight + DamageTileSize - 1) / DamageTileSize;
    convertedTiles = 0;
    totalTiles = columns * rows;
    for (int ty = 0; ty < rows; ++ty) {
        const int y0 = ty * DamageTileSize;
        const int y1 = std::min(AndroidHeight, y0 + DamageTileSize);
        for (int tx = 0; tx < columns; ++tx) {
            const int x0 = tx * DamageTileSize;
            const int x1 = std::min(AndroidWidth, x0 + DamageTileSize);
            const size_t rowBytes = static_cast<size_t>(x1 - x0) * BytesPerPixel;
            bool changed = false;
            for (int y = y0; y < y1 && !changed; ++y) {
                const size_t off = static_cast<size_t>(y) * AndroidStrideBytes +
                                   static_cast<size_t>(x0) * BytesPerPixel;
                changed = std::memcmp(source + off, prev + off, rowBytes) != 0;
            }
            if (!changed)
                continue;
            ++convertedTiles;
            for (int y = y0; y < y1; ++y) {
                for (int x = x0; x < x1; ++x) {
                    const size_t offset = static_cast<size_t>(y) * AndroidStrideBytes +
                                          static_cast<size_t>(x) * BytesPerPixel;
                    int luminance = (source[offset] * 77 + source[offset + 1] * 150 +
                                     source[offset + 2] * 29 + 128) >> 8;
                    if (inverted)
                        luminance = 255 - luminance;
                    const unsigned char value = quantizedLuminance(luminance, x, y, profile);
                    destination[offset] = destination[offset + 1] = destination[offset + 2] = value;
                    destination[offset + 3] = 0xff;
                }
            }
        }
                        ...
                        QImage &destination)
{
    ...
    std::chrono::steady_clock::time_point frameArrivedAt{};
    std::chrono::steady_clock::time_point frameDequeuedAt{};
    long long lastConvertMs = 0;
    int lastConvertedTiles = 0;
    int lastTotalTiles = 0;
    QByteArray previousRgbForTiles;
    QRect lastTinyRect;
    std::chrono::steady_clock::time_point lastTinyRectAt{};
    long long suppressedCaretFrames = 0;
    long long lastDiffMs = 0;
    uint64_t consumedPenPresenceGeneration = 0;
    uint64_t displayedFrames = 0;
    std::shared_ptr<QByteArray> deferredAndroidFrame;
    QRect overlayRepairRegion;  // [rmkit] 叠加层重置后需按区域修复的范围
    QRect scrollRepairRegion;  // [rmkit] 双指快滑走 Pen 波形的区域, 松手 settle 时用清晰波形重绘
    ...
    auto lastPenActivity =
        std::chrono::steady_clock::now() -
        std::chrono::seconds(10);
    auto lastRubberActivity =
        std::chrono::steady_clock::now() -
        std::chrono::seconds(10);  // [rmkit] 硬件橡皮最近活动
    auto lastFullRefreshAt =
        std::chrono::steady_clock::now() -
        std::chrono::seconds(10);  // [rmkit] 全屏刷新节流
    ...
    while (!stopRequested) {
        ...
        std::shared_ptr<QByteArray> frame;
        std::vector<QRect> frameDamage;
        bool frameDamageKnown = false;
        ...
        bool receiverEnded = false;
        {
            ...
            frameArrivedAt = mailbox.latestFrameArrivedAt;
            frameDequeuedAt = std::chrono::steady_clock::now();
            ...
            if (!mailbox.penSamples.empty()) {
                ...
                frameDamage = mailbox.damageRects;
                frameDamageKnown = mailbox.damageKnown;
                mailbox.consumedByDisplay = mailbox.receivedFrames;
            ...
            /*
             * Boot splash: until Android's first frame arrives (up to ~40 s
             * after the bridge starts) keep the panel showing the start-up
             * page with a cycling progress bar instead of blank white.
             */
            if (displayedFrames == 0 && !frame && penSamples.empty()) {
                static int splashTick = 0;
                static auto lastSplash = std::chrono::steady_clock::time_point::min();
                const auto now = std::chrono::steady_clock::now();
                if (now - lastSplash >= std::chrono::milliseconds(900)) {
                    lastSplash = now;
                    drawBootSplash(destination, 8 + (splashTick++ % 8), 16);
                    framebuffer->swapBuffers(
                        QRect(0, destination.height() / 2 + 100, destination.width(), 80),
                        EPScreenMode::Mono,
                        QFlags<EPFramebuffer::UpdateFlag>(EPFramebuffer::UpdateFlag::UIUpdate));
                }
            }        }
        ...
        const bool touchActive =
            ::access(TouchActivePath, F_OK) == 0;
        const bool scrollActive =
            ::access(ScrollActivePath, F_OK) == 0;
        ...
        if (interactiveSettleReady && !frame && haveLatestRgb) {
            ...
            if (touchSettlePending) {
                /*
                 * [rmkit v53] Animate 拖动的残影在松手时一次性记账
                 * (半个阈值): 会话帧逐帧豁免防的是拖动中途闪屏, 但
                 * Animate 恰是残影最重的波形, 不记账残影要攒 15 次
                 * 拖动才清理。现在 ~2 轮拖动 + 1.8s 空闲即触发一次
                 * 全屏清残影。
                 */
                /* [rmkit v54] 由 /2 放宽到 /4: 半阈值记账让连续阅读式
                 * 拖动每 2 次就闪一次清残影, 过于打扰 (跑分 S5 实测
                 * 三次 1.5-2.5s 全屏闪刷全是它)。1/4 阈值 = 约 4 轮
                 * 拖动一清, 残影控制与阅读连贯性的折中。 */
                accumulatedGhostArea = std::min(
                    PanelArea * GhostBudgetScreenMultiples * 2,
                    accumulatedGhostArea +
                        PanelArea * GhostBudgetScreenMultiples / 4);
            }
        ...
        /*
         * [rmkit] 交互会话 = 打字 / 双指滚动 / 手指按住拖动。
         * 会话期间整屏常驻 Animate 波形 + 转换直通 (波形自行量化)。
         * 点按类操作 (帧到达时手指已抬) 走常规画质路径。
         */
        const bool interactionSession =
            (typingActive || scrollActive || touchActive) &&
            !penOverlay.noteActive && !interactiveSettleFrame;
        ...
        if (::access(NoteOverlayResetPath, F_OK) == 0) {
            ...
            /*
             * [rmkit] 学习 stock: 彩笔 settle 只局刷笔迹区域。记下叠加层
             * 墨迹包围盒后按区域修复, 不再整屏 manualRefresh。
             */
            overlayRepairRegion =
                overlayRepairRegion.united(penOverlay.inkBounds);
            penOverlay.clear();
            pendingIdleCleanup = QRect();
        ...
        if (overlayResetPending) {
            ...
            writeLog(QStringLiteral(
                "native pen overlay reset applying HWC frame=%1 (regional)")
                         .arg(receivedFrames));
        }
        ...
        if (colorIdle) {
            ...
            if (!settled.isEmpty() && !duplicateColor) {
                ...
                if (activeColorMode == ColorMode::Auto) {
                    ...
                    nextAutoColorSettle = std::chrono::steady_clock::time_point::max();  // settled: wait for new content, do not re-poll
                }
                writeLog(QStringLiteral(
                ...
                nextAutoColorSettle = std::chrono::steady_clock::time_point::max();  // nothing chromatic: stop re-polling until new content arrives
            }
        ...
        if (frame && !manualRefresh) {
            ...
            if (interactiveFastPath && !typingActive &&
        ...
        if (frame) {
             ...
             */
            /*
             * [rmkit] 双指拖动: 即使阅读类应用在前台也走 Fast 快滑,
             * 松手后由既有的 interaction idle settle 重绘为清晰画面。
             */
            // [rmkit] Fast 抖动档退役: 交互速度由 Animate 会话保证。
            const DisplayProfile requestedProfile = selectedProfile;
            const bool requestedInvert = readInvertMode();
            const bool noteActive = readNoteActive();
            if (noteActive != penOverlay.noteActive ||
                requestedInvert != penOverlay.inverted) {
            ...
            const auto *androidPixels =
                reinterpret_cast<const unsigned char *>(
                frame->constData());
            /* [rmkit] v48: 会话帧跳过全屏色彩 settle 比对 (每帧 20-40ms
             * 的 memcmp); 会话结束后的 settle 帧照常走此路径恢复。 */
            const QRect rgbDirty = interactionSession
                ? QRect()
                : changedRectangle(androidPixels, latestRgb, !haveLatestRgb);
            latestRgb = *frame;
            haveLatestRgb = true;
            if (activeColorMode != ColorMode::Mono &&
                !rgbDirty.isEmpty()) {
                // RGB is submitted directly; no deferred colour settle needed.
                pendingColorSettle = QRect();
            ...
            /*
             * Send RGB straight through, as xochitl does.  The greyscale
             * "fast frame, colour later" split this replaces is what made
             * colour disappear and drove the idle colour-settle polling.
             * libqsgepaper handles waveform and colour per region itself.
             */
            // Diff on the raw RGB first, then convert only the dirty
            // rectangles; a full-panel convert costs 150-350 ms per frame.
            const auto diffStart = std::chrono::steady_clock::now();
            if (interactionSession) {
                /*
                 * [rmkit] xochitl 实测 (60s 双指拖动 1298 次提交 ≈21fps):
                 * 拖动期间不做增量 diff, 整个 animation 区每帧整体提交,
                 * Animate 波形便宜到不值得为省提交面积花 200-400ms 比对。
                 * 会话帧照抄: 全屏直通, diff 归零。
                 *
                 * [rmkit v50] 例外: 稀疏会话帧 (帧距 >300ms) 不是手势流,
                 * 而是"键盘开着没打字"时的光标闪烁 (2fps 整屏提交把
                 * SF/桥/波形生成全部烤热)。这类帧改走 diff, 让下游的
                 * caret 去重生效; 连续手势流 (帧距短) 不受影响。
                 */
                /*
                 * [rmkit v51] 纯打字会话 (无滚动/拖动) 一律走 diff:
                 * 字母/光标/候选栏都是小矩形, 整屏 Animate 提交只是在
                 * 烤机器 (QuickNote+输入法空闲实测 Trime/Markor 以
                 * ~10fps 无效化)。
                 * [rmkit v52] v50 的"稀疏帧走 diff"撤销: EinkBro 等重
                 * 页面滚动产帧间隔常 >300ms, 被误判成光标帧, 整屏
                 * Animate 平移被架空 (表现为两段式局刷)。滚动/拖动
                 * 会话无条件整屏直通; 光标发热场景由 typingOnly 覆盖
                 * (键盘收起后的光标帧本就走非会话 quality 路径+去重)。
                 */
                const bool typingOnly =
                    typingActive && !scrollActive && !touchActive;
                if (typingOnly) {
                    dirtyRectangles = changedRectangles(
                        androidPixels, previousRgbForTiles, !havePrevious);
                } else {
                    dirtyRectangles = {destination.rect()};
                }
            } else if (frameDamageKnown && havePrevious) {
                /*
                 * [rmkit] v47 HWC damage 透传 (混合策略): HWUI 在无
                 * partial-update 扩展的 SwiftShader 上把整窗报成 damage,
                 * 直接采用会放大提交面积; 故只用 damage 并集限定瓦片
                 * diff 的扫描范围 —— 空 damage 帧零成本跳过, 小窗口帧
                 * (输入法/对话框/焦点窗) 按包围盒缩小 diff, 整窗帧与
                 * 旧路径等价。
                 */
                if (frameDamage.empty()) {
                    ...
                    QRect damageBounds;
                    for (const QRect &rect : frameDamage)
                        damageBounds = damageBounds.isEmpty()
                            ? rect
                            : damageBounds.united(rect);
                    dirtyRectangles = changedRectangles(
                        androidPixels, previousRgbForTiles, false,
                        damageBounds);
                }
            } else {
                dirtyRectangles = changedRectangles(
            ...
            lastDiffMs = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - diffStart).count();
            if (converted.size() != static_cast<qsizetype>(VisibleBytes))
                converted.resize(static_cast<qsizetype>(VisibleBytes));
            /*
             * [rmkit] 打字会话转换直通: Animate 波形自会量化灰阶, 预先的
             * 4 灰阶转换 (大区域 100-300ms) 是重复劳动。直接按行拷贝原始
             * RGB 进提交缓冲; 反色模式仍需转换 (否则闪出未反色画面)。
             * 停止打字后 settle 帧走常规转换恢复画质与对比设置。
             */
            /*
             * [rmkit] 交互会话 = 打字 / 双指滚动 / 手指按住拖动。
             * 会话期间整屏常驻 Animate 波形, 转换直通 (波形自行量化),
             * 我们不再自己做 Fast 抖动。点按类操作 (帧到达时手指已抬)
             * 走常规画质路径, 避免"先糊后清"的二次重绘。
             */
            const bool typingPassthrough =
                interactionSession && !activeInvert;
            if (!typingPassthrough) {
                for (const QRect &r : dirtyRectangles)
                    convertColorForEink(
                        androidPixels, converted, r, activeInvert);
            }
            /* [rmkit] 会话帧零暂存: 直接从原始帧上屏, 不经 converted。 */
            lastConvertedTiles = static_cast<int>(dirtyRectangles.size()); lastTotalTiles = 0;
            lastConvertMs = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - convertStart).count() - lastDiffMs;
            const auto *pixels = typingPassthrough
                ? androidPixels
                : reinterpret_cast<const unsigned char *>(
                      converted.constData());
            ...
            /*
             * [rmkit] 学习 stock 的判断: 黑色墨迹写完不补刷。笔离开后释放
             * 的帧若其脏区新旧内容均无彩色像素、且不落在笔记 UI 区域, 则
             * 跳过提交 —— 原生速墨即为定稿 (与 stock 黑笔一致); 彩色笔迹
             * 仍照常重绘 (单色速墨需要换成 Gallery 3 彩色)。橡皮擦除是
             * 无彩差异但必须重绘, 故擦除工具激活时不过滤。基线照常前移,
             * 被跳过的墨迹区域从此视为已上屏, 不会再被后续 diff 重提。
             */
            if (penOverlay.noteActive && !manualRefresh &&
                !interactiveSettleFrame && !dirtyRectangles.empty() &&
                std::chrono::steady_clock::now() - lastPenActivity <
                    std::chrono::milliseconds(4000) &&
                std::chrono::steady_clock::now() - lastRubberActivity >
                    std::chrono::milliseconds(4000)) {
                const NotePenSettings inkSettings = readNotePenSettings();
                const auto *previousPixels =
                    previousRgbForTiles.size() == latestRgb.size()
                        ? reinterpret_cast<const unsigned char *>(
                              previousRgbForTiles.constData())
                        : nullptr;
                if (!inkSettings.appEraser) {
                    std::vector<QRect> keptRectangles;
                    qint64 skippedArea = 0;
                    for (const QRect &damage : dirtyRectangles) {
                        bool uiRect =
                            damage.top() <
                                static_cast<int>(inkSettings.uiBottom);
                        if (!uiRect) {
                            for (const QRectF &region
                                 : inkSettings.uiRegions) {
                                if (!region.isEmpty() &&
                                    damage.intersects(
                                        region.toAlignedRect())) {
                                    uiRect = true;
                                    break;
                                }
                            }
                        ...
                        const bool chromatic =
                            !chromaticRectangles(
                                 androidPixels, damage).empty() ||
                            (previousPixels &&
                             !chromaticRectangles(
                                  previousPixels, damage).empty());
                        if (uiRect || chromatic) {
                            keptRectangles.push_back(damage);
                        } else {
                            skippedArea +=
                                static_cast<qint64>(damage.width()) *
                                damage.height();
                        }
                    }
                    if (keptRectangles.size() != dirtyRectangles.size()) {
                        writeLog(QStringLiteral(
                                     "black ink no-settle kept=%1 "
                                     "skipped=%2 area=%3")
                                     .arg(keptRectangles.size())
                                     .arg(dirtyRectangles.size() -
                                          keptRectangles.size())
                                     .arg(skippedArea));
                        dirtyRectangles = std::move(keptRectangles);
                        dirty = rectanglesBoundingRect(dirtyRectangles);
                        if (dirtyRectangles.empty() &&
                            !toolbarRefreshPending) {
                            /* 速墨已在屏上; 只推进基线, 不上屏。 */
                            previousRgbForTiles = latestRgb;
                            havePrevious = true;
                            consumedFrames = receivedFrames;
                            QCoreApplication::processEvents();
                            continue;
            ...
            {
                /*
                 * A focused text field blinks its caret every 500 ms; each
                 * blink is a tiny identical frame that still costs a convert
                 * and an e-ink write, queued ahead of real keystrokes.
                 * Suppress repeats of the same small rectangle within 1.5 s.
                 */
                const auto tinyNow = std::chrono::steady_clock::now();
                const bool tinyRepeat =
                    dirtyRectangles.size() == 1 &&
                    dirty == lastTinyRect &&
                    dirty.width() * dirty.height() <= 64 * 160 &&
                    tinyNow - lastTinyRectAt < std::chrono::milliseconds(700) &&
                    tinyNow - lastTinyRectAt > std::chrono::milliseconds(300);  // caret period ~500ms; keystrokes are faster or slower
                lastTinyRect = dirty;
                lastTinyRectAt = tinyNow;
                if (tinyRepeat) {
                    ++suppressedCaretFrames;  // converted unchanged; previous already valid
                    if (suppressedCaretFrames % 20 == 1)
                        writeLog(QStringLiteral("caret skip #%1 rect=%2,%3 %4x%5")
                                     .arg(suppressedCaretFrames).arg(dirty.x()).arg(dirty.y())
            ...
            if (!overlayRepairRegion.isEmpty()) {
                /* 叠加层墨迹可能比 Android 笔迹略宽, 强制重绘其范围。 */
                addDamageRectangle(dirtyRectangles, overlayRepairRegion);
                dirty = rectanglesBoundingRect(dirtyRectangles);
                overlayRepairRegion = QRect();
            }
            if (interactiveSettleFrame && !scrollRepairRegion.isEmpty()) {
                /*
                 * [rmkit] 快滑期间用 Pen 波形上屏的区域在 RGB 层无差异,
                 * 普通 diff 不会重绘; settle 帧强制用清晰波形补一遍。
                 */
                addDamageRectangle(dirtyRectangles, scrollRepairRegion);
                dirty = rectanglesBoundingRect(dirtyRectangles);
                scrollRepairRegion = QRect();
            }
            ...
            if (!dirty.isEmpty()) {
                ...
                // No new Android frame (note-mode switch, manual refresh).
                // Only a genuine manual refresh should repaint the whole
                // panel; `converted` is incrementally updated per dirty rect
                // since v18, so a blind full-rect submit here pushed a stale
                // buffer and produced the blank notebook page.
                if (manualRefresh) {
            ...
            // Keep the RGB diff baseline in step with what actually reached
            // the panel; updating it before a frame could still be skipped
            // (caret dedupe, note-mode switch) made the next identical frame
            // diff to nothing and left a blank page until the pen moved.
            previousRgbForTiles = latestRgb;  // full RGB of the frame that reached the panel
            havePrevious = true;
        }
        ++displayedFrames;
        ...
        /*
         * [rmkit] 全屏刷新节流: 应用启动/窗口过渡会连发多次全屏刷新请求,
         * 每次 1.6-1.9s 且 sync 阻塞, 连发时十余秒不可交互。3 秒冷却期内
         * 的重复请求降级为普通局部提交 (不闪、不阻塞), 视觉上内容照样更新。
         */
        if (fullUpdate && displayedFrames != 1 &&
            submitStarted - lastFullRefreshAt < std::chrono::seconds(3)) {
            fullUpdate = false;
            writeLog(QStringLiteral("full refresh throttled (cooldown)"));
        }
        if (fullUpdate)
            lastFullRefreshAt = submitStarted;
        ...
        if (fullUpdate) {
            // Automatic ghost cleanup flashes the whole panel to black with
            // the Content waveform.  Use Grayscale for that case (no black
            // flash, slightly weaker ghost removal); keep Content for
            // explicit user-requested refreshes.
            framebuffer->swapBuffers(
                destination.rect(),
                automaticGhostRefresh ? EPScreenMode::Grayscale
                                      : EPScreenMode::Content,
                QFlags<EPFramebuffer::UpdateFlag>(
                    EPFramebuffer::UpdateFlag::UIUpdate |
                    EPFramebuffer::UpdateFlag::FullUpdate));
            ...
            const QRect navigationRect(
                ...
                AndroidWidth, NavigationBarHeight);
            /*
             * Submit every dirty rectangle in one classified update and let
             * libqsgepaper pick the waveform per region, exactly as xochitl
             * does.  The per-rectangle Mono/Animate/Grayscale selection this
             * replaced fought the library's own auto-waveform logic and was
             * the source of the fast-profile streaks and the "fast vs quality"
             * trade-off; the library already handles that per region.
             */
            if (!dirtyRectangles.empty()) {
                const QSize panelSize(AndroidWidth, AndroidHeight);
                EPScreenModeMap modeMap(panelSize, {});
                QRegion dirtyRegion;
                if (interactionSession) {
                    /*
                     * [rmkit] 照抄 stock 输入法 (pinyin_ime.qml fastZone):
                     * 打字会话期间整屏常驻 Animate 波形 + 单色内容类型,
                     * 每帧模式图内容完全相同 —— 模式图每变一次就整屏重
                     * 合成一次, 常驻不变才零抖动; 文字灰阶由 Animate 波形
                     * 量化, 不做 1-bit 预抖动。停止打字后 settle 用常规
                     * 波形重绘恢复画质 (scrollRepairRegion)。
                     */
                    const QRect panelRect(0, 0, AndroidWidth, AndroidHeight);
                    modeMap.setModeForRegion(
                        QRegion(panelRect), EPScreenMode::Animate);
                    for (const QRect &damage : dirtyRectangles) {
                        scrollRepairRegion =
                            scrollRepairRegion.united(damage);
                        dirtyRegion += damage;
                    }
                } else {
                for (const QRect &damage : dirtyRectangles) {
                    ...
                    const bool penUpdate =
                        !typingActive && regionArea * 25 <= PanelArea &&
                        !damage.intersects(QRect(0, 0, AndroidWidth, ToolbarHeight)) &&
                        !damage.intersects(QRect(0, AndroidHeight - NavigationBarHeight,
                                                 AndroidWidth, NavigationBarHeight));
                    const bool scrollUpdate =
                        scrollActive && !typingActive &&
                        !penOverlay.noteActive && !interactiveSettleFrame;
                    if (penUpdate)
                        ++penRegionCount;
                    if (penUpdate || scrollUpdate)
                        modeMap.setModeForRegion(QRegion(damage), EPScreenMode::Pen);
                    if (scrollUpdate)
                        scrollRepairRegion = scrollRepairRegion.united(damage);
                    dirtyRegion += damage;
                }
                }
                framebuffer->swapBuffers(
        ...
        if (readerCleanupFrame) {
            ...
            readerPagesSinceCleanup = 0;
            accumulatedGhostArea = 0;
        } else if (!fullUpdate && !interactionSession) {  // [rmkit] Animate 会话帧不计残影预算 (波形无残影累积; 会话后 settle 已重绘), 否则拖几下预算即爆引发 2.5-3.9s 全屏闪刷打断交互
            accumulatedGhostArea = std::min(
                PanelArea * GhostBudgetScreenMultiples * 2,
        ...
        if (true) {  // [latency probe] log every frame
            writeLog(QStringLiteral(
                          "coalesced frame queue_ms=%15 total_ms=%16 convert_ms=%17 diff_ms=%21 dmg=%22 tiles=%18/%19 caret_skip=%20 received=%1 displayed=%2 "
                         ...
                         .arg(dirty.y())
                          ...
                          .arg(std::chrono::duration_cast<std::chrono::milliseconds>(frameDequeuedAt - frameArrivedAt).count())
                          .arg(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - frameArrivedAt).count())
                          .arg(lastConvertMs)
                          .arg(lastConvertedTiles)
                          .arg(lastTotalTiles)
                          .arg(suppressedCaretFrames)
                          .arg(lastDiffMs)
                          .arg(frameDamageKnown
...
int main(int argc, char **argv)
{
    ...
    loadUiLocale();
    writeLog(QStringLiteral("rm-epd-bridge v55 (fw 5.8.x EP API) start; native HWC path, no VNC"));
    ...
    for (int second = 0; second < 8 && !stopRequested; ++second) {
        drawBootSplash(destination, second, 8);
        framebuffer->swapBuffers(
            destination.rect(),
            second == 0 ? EPScreenMode::Grayscale : EPScreenMode::Mono,
            QFlags<EPFramebuffer::UpdateFlag>(EPFramebuffer::UpdateFlag::UIUpdate));
        ::sleep(1);
    }
    ...
    while (!stopRequested) {
        ...
        if (!dirty.isEmpty()) {
            ...
            // [rmkit] 旧 diff 基线缓冲 previous 只写不读, 已删除 (每帧省 6.5MB memcpy)
            havePrevious = true;
            ...
            const qint64 fullArea =
                static_cast<qint64>(AndroidWidth) * AndroidHeight;
            // 本地调优（2026-08-26）：实测 submit_ms 在 Content 波形下为 1400~1700ms，
            // 在 Animate 波形下仅 5~124ms，相差约 300 倍。而原策略只要脏区超过屏幕
            // 60% 就走 Content —— Android 的界面切换、列表滚动动辄整屏重绘，于是
            // 几乎每次操作都要等 1.4 秒才看到结果，这正是"操作发闷"的直接来源
            // （同期 CPU 几乎全空闲，说明瓶颈不在算力而在刷新波形选择）。
            //   periodicFull: 每 30 帧强制全刷 → 放宽到 120 帧
            //   largeUpdate:  阈值 60% → 92%，只有近乎整屏重绘才用慢波形
            // 代价是残影累积更快，由 periodicFull 的周期性全刷负责清理。
            const bool periodicFull = displayedFrames % 120 == 0;
            const bool largeUpdate = dirtyArea * 100 >= fullArea * 92;
            ...
            // 进一步调优：实测快波形(Animate) submit_ms 为 0~51ms，慢波形(Content)
            // 恒为 1400~1750ms。Android 的界面切换几乎总是整屏重绘，只要让
            // largeUpdate 参与波形选择，就必然落到慢波形上 —— 这正是"点一下等一秒多"
            // 的来源。这里让 largeUpdate 只影响刷新**区域**（整屏提交，避免撕裂），
            // 但**不再影响波形**：除周期性全刷外一律用快波形。
            // 代价是残影累积更明显，由每 120 帧一次的 periodicFull 负责清理。
            framebuffer->swapBuffers(
