...
static volatile sig_atomic_t orderly_kill_requested = 0;
static void on_orderly_kill(int sig) { (void)sig; orderly_kill_requested = 1; }
...
static void stop_child_for_orderly_reboot(pid_t child, const char *name)
{
    ...
    log_message("orderly stock reboot: %s did not exit after SIGKILL; "
                "continuing", name);
}
...
static bool stock_boot_is_already_committed(void)
{
    ...
    (void)boot_part; /* 单槽: eMMC 启动分区与本槽无关 */
    if (error_count != '0') {
...
{
    /*
     * [rmkit] reDroid 12 镜像没有 NXP 的 A12 vendor Wi-Fi HAL 与 HIDL
     * wpa_supplicant，Android 自己开不了 Wi-Fi。改为宿主接管：wlan0 改名为
     * eth0（匹配 Android config_ethernet_iface_regex "eth\d"），由宿主的
     * stock wpa_supplicant 用用户已保存的网络完成关联，Android 的
     * EthernetService 在 netd 启动时枚举到 eth0 并对其做 DHCP / 注册网络。
     * 改名必须发生在 Android 启动之前，netd 只在启动时枚举接口。
     */
    char *const wlan_down[] = {
        "/sbin/ip", "link", "set", "wlan0", "down", NULL
    };
    char *const wlan_rename[] = {
        "/sbin/ip", "link", "set", "wlan0", "name", "eth0", NULL
    };
    char *const eth_up[] = {
        "/sbin/ip", "link", "set", "eth0", "up", NULL
    };
    ...
    char *const supplicant_with_networks[] = {
        ...
        "-P", "/run/wpa_supplicant-eth0.pid", NULL
    };
    char *const supplicant_bare[] = {
        "/usr/sbin/wpa_supplicant", "-B", "-Dnl80211", "-ieth0",
    ...
    if (!command_succeeded(run_command(wlan_down)) ||
        !command_succeeded(run_command(wlan_rename)) ||
        !command_succeeded(run_command(eth_up))) {
    ...
    if (access(networks, R_OK) == 0) {
        if (!command_succeeded(run_command(supplicant_with_networks)))
            ...
            log_message("host wpa_supplicant started on eth0 with saved networks");
    } else {
        if (!command_succeeded(run_command(supplicant_bare)))
            log_message("host wpa_supplicant failed to start");
        else
            log_message("host wpa_supplicant started on eth0 (no saved networks)");
    }
    log_message("NXP IW61x exposed to Android as eth0 (host-managed Wi-Fi)");
    return true;
}
...
{
    /* 单槽改造: 不切 eMMC 启动分区, 也不 fork 任何子进程 (Android 退出时内存紧张会 ENOMEM)。*/
    char boot_part = '\0';
    ssize_t read_count;
    int fd;
    ...
    (void)boot_part; /* 单槽: 只清计数, 不校验启动分区 */

    sync();
    log_message("%s: stock boot committed (single-slot, errcnt=0)",
                reason);
    return true;
}
...
{
    /*
     * [rmkit] performance 会把 CPU 锁在 1.7GHz 常驻, 机身发烫。改用 ondemand
     * 自动调频: 空闲 900MHz, 负载超过 40% 立即升频, 触摸响应几乎无差别。
     */
    write_text_file(
        "/sys/devices/system/cpu/cpufreq/policy0/scaling_governor",
        "ondemand\n");
    write_text_file("/sys/devices/system/cpu/cpufreq/ondemand/up_threshold",
                    "40\n");
    write_text_file("/sys/devices/system/cpu/cpufreq/ondemand/io_is_busy",
                    "1\n");
    write_text_file(
        "/sys/devices/system/cpu/cpufreq/ondemand/sampling_down_factor",
        "2\n");
    write_text_file("/sys/block/mmcblk0/queue/read_ahead_kb", "256\n");
    log_message("Android ondemand cpufreq governor enabled");
}
...
static void prepare_android_mounts(void)
{
    ...
    /* 单槽改造: android 系统在 /home/root/android-system (当前槽 rootfs 只有 452MB,
     * 装不下 android 树)。/home 已在 main 早段挂好, 先把它绑到 /android。
     * 原双槽版这里靠 root_b 槽 rootfs 自身就是 android 树, 单槽没有, 故显式绑定。*/
    make_dir(ANDROID_ROOT, 0755);
    if (mount("/home/root/android-system", ANDROID_ROOT, NULL, MS_BIND | MS_REC, NULL) < 0)
        log_message("single-slot: bind /home/root/android-system -> %s failed: %s",
    ...
    /*
     * [rmkit] 把宿主 /home/root 只读挂进 Android 共享存储
     * (文件管理器路径: 内部存储/rmppm-home)。宿主挂载全是 private 传播,
     * 运行时从宿主挂不进 Android 命名空间; 必须在这里、在下面对
     * ANDROID_DATA 的递归 bind 之前就挂好, 让 MS_REC 把它一起带进去。
     */
    /*
     * [rmkit 单槽] 全新数据目录上 Android 还没首启, /data/media/0 不存在; 这里不能替
     * Android 建它 (属主/模式由 vold/installd 定), 更不能因此退出 (make_dir 会 _exit)。
     * 目录不存在就跳过, 首启之后下次进 Android 自然挂上。
     */
    snprintf(path, sizeof(path), "%s/media/0", ANDROID_DATA);
    if (access(path, F_OK) != 0) {
        log_message("rmppm-home bind skipped: %s not created yet (first boot)", path);
    } else {
        snprintf(path, sizeof(path), "%s/media/0/rmppm-home", ANDROID_DATA);
        if (mkdir(path, 0755) < 0 && errno != EEXIST) {
            log_message("rmppm-home mkdir failed: %s", strerror(errno));
        } else if (mount("/home/root", path, NULL, MS_BIND, NULL) == 0) {
            if (mount(NULL, path, NULL,
                      MS_REMOUNT | MS_BIND | MS_RDONLY, NULL) < 0)
                log_message("rmppm-home ro remount failed: %s", strerror(errno));
            log_message("host home exposed read-only at media/0/rmppm-home");
        } else {
            log_message("rmppm-home bind failed: %s", strerror(errno));
        }
    }
...
int main(void)
{
    ...
    const char *const android_modules[] = {
         ...
         */
        "loop", "ashmem_linux", "zram", "max77818_charger", "ci_hdrc_imx", "fusb303b",
        /*
    ...
    /* 单槽改造: 绝不动 eMMC 启动分区 (bootpart 1 = 下次启动 root_a, 会离开本槽);
     * 回落由 /sbin/init 包装在开机时复位内核链接完成。保留一次无害命令维持流程。*/
    char *const arm_stock_fallback[] = { "/bin/true", NULL };
    char *const android_argv[] = {
        ...
        /*
         * 本地调优（2026-08-26）：实测滑动时 SystemUI 43.7% + SurfaceFlinger 37.5% CPU
         * （双核里一个核吃满），而 EPD 桥统计 received=2171 / displayed=340 ——
         * 送来的帧有 84% 被丢弃，因为墨水屏物理上一秒刷不了 30 次。
         * 按 30fps 合成纯属浪费算力，降到 10fps 后合成次数减少三分之二，
         * 实际能显示的帧数不变。
         */
    ...
    for (size_t index = 0; android_modules[index] != NULL; ++index) {
        ...
        /*
         * [rmkit] devtmpfs 默认给 ashmem 节点 0600, 应用 (uid 10071 等)
         * 打不开, moai sqlite 的 CursorWindow 分配即失败。ueventd 的
         * coldboot 补 0666 存在竞态, 这里在加载后立即固定权限。
         */
        if (strcmp(android_modules[index], "ashmem_linux") == 0)
            (void)chmod("/dev/ashmem", 0666);
    ...
    /*
     * reDroid's init acknowledges sys.powerctl=shutdown but never exits
     * (HandlePowerctlMessage is a no-op inside the container), so the
     * orderly stock-reboot handshake below stalls forever on waitpid().
     * Watch for the orderly marker from a helper and, after a grace
     * period for Android to flush, terminate the namespace so the
     * existing exit path takes over.
     */
    {
        pid_t grace = fork();
        if (grace < 0) {
            log_message("orderly grace watchdog: fork failed: %s",
                        strerror(errno));
        } else if (grace == 0) {
            for (;;) {
                if (access(STOCK_ORDERLY_MARKER, R_OK) == 0) {
                    log_message("orderly grace watchdog: marker seen, "
                                "allowing 15 s for Android to exit");
                    sleep(15);
                    /* android_child is in a child PID namespace; only the
                     * host-side parent can address it. */
                    log_message("orderly grace watchdog: asking parent to "
                                "terminate namespace init");
            ...
            log_message("orderly grace watchdog started (host pid=%ld)",
                        (long)grace);
        }
    }
    signal(SIGUSR1, on_orderly_kill);
    for (;;) {
        pid_t exited = waitpid(-1, &child_status, 0);
        if (orderly_kill_requested && android_child > 0) {
            orderly_kill_requested = 0;
            log_message("orderly stock reboot: grace expired; "
                        "terminating namespace init");
            kill(android_child, SIGKILL);
        }
        if (exited < 0) {
            if (errno == EINTR)
                continue;
    ...
    if (orderly_stock_requested) {
        ...
        if (!detach_android_host_mounts_for_reboot()) {
            /*
             * Detach can fail when a straggling host helper still holds a
             * path under /android.  The boot partition has already been
             * switched, so a hard reboot lands in stock regardless; parking
             * in pause() here left the panel on "restarting" indefinitely.
             */
            log_message("orderly stock reboot: mount detach failed; "
                        "forcing reboot anyway");
            sync();
            sleep(2);
            reboot(RB_AUTOBOOT);
        }
