...
/*
 * 6.12 移植说明：shmem_zero_setup() 未 EXPORT_SYMBOL，模块不可用。
 * 该调用在此处的实际作用仅是设置 vma->vm_ops（其设置的 vm_file 随后被
 * vma_set_file() 覆盖），因此改为借用 shmem 自身的 mmap 达到等效效果。
 */
static int (*ashmem_shmem_mmap)(struct file *, struct vm_area_struct *);
...
static int ashmem_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (!asma->file) {
		...
		if (!vmfile_fops.mmap) {
			...
			ashmem_shmem_mmap = vmfile->f_op->mmap;
	...
	if (vma->vm_flags & VM_SHARED) {
		ret = ashmem_shmem_mmap ?
			ashmem_shmem_mmap(asma->file, vma) : -EINVAL;
		if (ret) {
			fput(asma->file);
			goto out;
		}
...
MODULE_DESCRIPTION("Android ashmem driver (ported to Linux 6.12)");
